...
**Envoyé le 16/09 à 15:19 (Paris)** par `scripts/inviter-fondateurs.ts --vraiment`, après un
test sur l'adresse de Mathias à 15:17 (« reçu, c'est bon »). **13 remis sur 13** (Brevo, aucun
rejet, Hotmail/Yahoo/Orange compris). Mathias (nº2) l'a reçu deux fois (test + campagne).
Le C0 a été refondu le matin même (PR #152, template Brevo 44 repoussé, borné à C0) : sujet
« L'atelier est en ligne », image d'en-tête (les magazines de l'accueil, champ facultatif `hero`
de la maquette, absent des quinze autres mails dont le HTML est identique à l'octet près), bouton
« Composer mon magazine », lien inchangé vers `/compte/bienvenue?e=` (sans expiration).

**État à 15:45** : ouverts par Merisa (nº5), Klervie (nº13) et Mathias ; Merisa a cliqué et demandé
son lien à 15:28 (C2 remis chez Hotmail à 15:29, pas encore ouvert). Aucune connexion ni dossier
de fondateur ce jour. Les dossiers de test de Mathias (« Test math 16/09 », « Test vendredi
LYON ») sont supprimés : **plus aucun dossier sous son adresse.** Le prochain point : le 17/09
au matin ; relance un par un (`--seulement <email>`) dans deux ou trois jours pour les non-ouverts,
sur accord de Mathias.

**Deux portes réparées le jour même (PR #153, en production à 15:33)**, trouvées sur le compte
de Mathias (journal auth : `422 same password` puis `403 token not found`) :
1. « Créer mon compte » sur une adresse déjà inscrite envoie désormais C2 au lieu de rien
   (`envoyerC1Inscription` → `compteExisteDeja` → `envoyerC2Reinitialisation`) ; réponse HTTP
   inchangée, anti-énumération conservée. Prouvé en local sur l'adresse de Mathias
   (`generate_link` signup 422 puis recovery 200, envoi coupé par `ATELIER_MAILS_COUPES`).
2. Retaper son mot de passe actuel sur `/compte/reinitialiser` affiche « C'est déjà votre mot de
   passe actuel, connectez-vous avec » (`meme_mot_de_passe`) au lieu d'« essayez-en un autre »,
   qui menait droit sur « lien déjà servi » (le lien est dépensé par `verifyOtp` avant `updateUser`).
   Non rejoué de bout en bout : il faudrait taper le mot de passe existant d'un vrai compte.

**Vérifié en base le 16/09** : les 12 comptes pré-créés le 07/09 portent l'empreinte d'un mot de
passe VIDE (Supabase hache la chaîne vide à la création sans mot de passe) : le cas « même mot
de passe » leur est impossible. Seuls Mathias (Google + mot de passe) et Merisa (connectée le
10/09) ont un vrai mot de passe. La page de bienvenue porte déjà « J'ai déjà mon mot de passe ».

`scripts/supprimer-dossiers.ts` accepte `--titre "…"` (PR #154) pour ne jeter qu'un dossier.
...
blanc.
 : grille HT, zones de port, exemplaires — EN PRODUCTION
