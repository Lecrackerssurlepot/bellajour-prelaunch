...
| T-126 | Le lien Canva partagé se publie sans que personne ait vu vers quel design il mène | admin | bloquant | libre | en cours (22/09, PR #201 EN PROD) : vu en prod chez Marjorie, `canva_url` → « Réferences » ; le dry-run nomme le design (`canva.ts` + `canvaDistant.ts`, harnais +21) ; lien de Marjorie corrigé et republié le 22/09 10:16, preuve prod faite (« Ce lien ouvre « Marjorie » »), mail d'excuse parti (deux fois, par erreur) ; plus rien à faire, à fermer |
| T-127 | Un lien Canva en édition, en lecture seule ou fermé passe la publication de la maquette | admin | serieux | libre | en cours (22/09, PR #201 EN PROD) : ⚠️ le chemin `/edit` ne prouve rien, c'est le rôle de la liste d'accès qui décide ; EDITOR / VIEWER / NONE / non partagé / sans extension refusés en 422 ; reste la preuve prod au premier refus réel |
