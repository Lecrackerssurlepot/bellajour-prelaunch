...
| T-121 | La fiche doit recouper elle-même l'export Canva en pages d'impression, l'atelier ne peut pas passer par un script à chaque dossier | atelier | serieux | avis-requis | nouveau (18/09) — rien ne découpe aujourd'hui, le dépôt part tel quel chez Cloudprinter ; découpe faite à la main pour Merisa ; proposition : découpe dans le navigateur avant dépôt (pdf-lib), module pur `decoupe.ts` au harnais, étape 1 de T-078 ; + bloquer l'envoi sur un verdict hors format, + résolution réelle des photos posées au dépôt et alerte photos trop petites sur la fiche |
