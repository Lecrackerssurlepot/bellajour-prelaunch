...
| T-133 | Deux lots dans deux ordres s'écrivaient dans le même dossier, numérotations mêlées | admin | serieux | libre | en cours (22/09, branche `fix/dossier-par-ordre`) : `nomDossier` ignorait l'ordre, un lot par date s'AJOUTAIT au lot du dépôt (préfixes différents, rien d'écrasé) ; le dossier porte désormais l'ordre (« … (par date) ») ; le tri lui-même était juste |
| T-132 | Le lieu sous le jour sur chaque vignette de la fiche | admin | confort | libre | en cours (22/09, branche `feat/lieu-sur-vignette`) : le voile de la vignette prend une seconde ligne, la ville sinon le pays ; le rang de dépôt proposé a été refusé par Mathias |
