...
| T-126 | Le lien Canva partagé se publie sans que personne ait vu vers quel design il mène | admin | bloquant | libre | en cours (22/09, PR #201 EN PROD) : vu en prod chez Marjorie, `canva_url` → « Réferences » ; le dry-run nomme le design (`canva.ts` + `canvaDistant.ts`, harnais +21) ; **reste la correction du lien de Marjorie par Mathias dans la fiche** et la preuve prod à la première publication |
