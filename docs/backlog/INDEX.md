...
| T-130 | La grille de la fiche par sous-groupes de lieu, et le lot qui suit | admin | confort | libre | en cours (22/09, branche `fix/lot-par-date`) : bouton « Par lieu », **deuxième passe l'après-midi** : un groupe par SÉJOUR dans l'ordre du temps (nouveau groupe à chaque changement de ville, sinon pays ; une datée sans lieu reste dans le séjour en cours), « Sans date » en queue dans l'ordre du dépôt, titre = lieu + compte + période (`grouperParLieu`, pur) ; le lot descend dans le même ordre (`ordre: "lieu"`) ; harnais +7 |
