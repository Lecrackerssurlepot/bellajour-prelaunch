...
| T-134 | Après des retouches, le client reçoit M5 mot pour mot, sans un mot sur les corrections qu'il a demandées | atelier | serieux | avis-requis | en cours (23/09, PR #214 EN PROD) : un SECOND GABARIT pour le code M5 (« vos corrections sont faites », template Brevo 46, variable posée par Mathias), donc verrou, garde-fou de chaîne et auto-validation intacts ; le repli sur le gabarit M5 ordinaire NEUTRALISE le piège du mail sauté en silence ; case « j'ai corrigé » dans l'atelier, pré-cochée quand le client a cliqué, et cochable à la main quand il a écrit dans le Canva sans cliquer (demande de Mathias) ; bouton « Republier », ligne de confirmation recalculée sur la case et non sur la projection serveur ; harnais +5. **Reste à prouver sur une republication RÉELLE** |
