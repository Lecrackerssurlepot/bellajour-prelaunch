...
| T-134 | Après des retouches, le client reçoit M5 mot pour mot, sans un mot sur les corrections qu'il a demandées | atelier | serieux | avis-requis | nouveau (23/09) — VÉRIFIÉ à l'ouverture : la republication supprime le verrou M5 et renvoie le MÊME gabarit, seul `DATE_LIMITE` change (`mails.ts:607`) ; objet identique, donc empilé dans le même fil Gmail. Les deux textes sont validés par Mathias. Chemin retenu : garder le code M5 et choisir le GABARIT à l'envoi (repli sur M5 normal si l'identifiant manque : le repli NEUTRALISE le piège du mail sauté en silence, vérifié). ⚠️ Le drapeau ne peut PAS passer par `prepa.params` (`templatePour` est appelé avant que `extra` ne soit fusionné) ni se déduire du dossier (`retouches_demandees_le` est déjà remis à null quand le mail part) : il passe par `options` et `releverDossier` doit le relayer, plus trois libellés d'admin qui distinguent enfin publier de republier. **Attend Mathias** : gabarit Brevo à créer, variable Vercel à poser |
