...
| T-126 | Le lien Canva partagé se publie sans que personne ait vu vers quel design il mène | admin | bloquant | libre | en cours (22/09, branche `fix/canva-lien-garde-fous`) : vu en prod chez Marjorie, `canva_url` → « Réferences » ; le dry-run nomme le design (`canva.ts` + `canvaDistant.ts`, harnais +21) ; reste la fusion et la correction du lien par Mathias dans la fiche |
| T-127 | Un lien Canva en édition, en lecture seule ou fermé passe la publication de la maquette | admin | serieux | libre | en cours (22/09, même branche que T-126) : ⚠️ le chemin `/edit` ne prouve rien, c'est le rôle de la liste d'accès qui décide ; EDITOR / VIEWER / NONE / non partagé / sans extension refusés en 422 ; reste la fusion |
