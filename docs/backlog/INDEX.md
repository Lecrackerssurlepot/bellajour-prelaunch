...
| T-116 | M0 « il attend vos photos » arrive à la seconde, pendant que le client est encore en train de déposer | atelier | serieux | avis-requis | en cours (17/09, branche `feat/m0-differe` : M0 programmé chez Brevo pour +15 min à la création, retiré de la file à la première photo ou au clic « Envoyer » ; journal `mail_programme` / `mail_annule` ; module pur `programme.ts`, 19 assertions ; texte de M0 réécrit) — **en prod le 18/09** (PR #162), template M0 poussé ; reste le premier dossier réel pour voir l'annulation dans le journal |
