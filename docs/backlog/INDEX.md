...
| T-107 | Les mails mettent 5 à 9 minutes à arriver, alors que le site les soumet à la seconde | atelier | serieux | avis-requis | nouveau (11/09) — mesuré : 5 min 07 s à 8 min 57 s entre `requests` et `delivered` chez Brevo ; quota, DNS et rebonds écartés ; compte en plan gratuit. Attend la décision de passer au payant |
| T-108 | Un test automatisé a créé un vrai dossier et envoyé un vrai mail | atelier | serieux | libre | nouveau (11/09) — incident du jour, dégât nul (adresse de Mathias, dossier supprimé). Propose un interrupteur `ATELIER_MAILS_COUPES` dans `sendBrevoEmail` |
